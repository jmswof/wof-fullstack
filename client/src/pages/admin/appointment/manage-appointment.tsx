...
const ManageAppointment: React.FC = () => {
  ...
  useEffect(() => {
    ...
    .then(([ustates, references, priorities, colors, floorTypes, saleAgents, appointment]) => {
      ...
      setFloorType(
        floorTypes.filter(fts => appointment['floorType'].some(ats => fts['short'] === ats)).map(ft => ft.short)
      );
      setReference(
        references.filter(r => appointment['reference'].some(ar => r['short'] === ar)).map(r => r.short)
      );
      setInternalNotes(appointment['internalNotes']);
      setSalesNotes(appointment['salesNotes']);
      setColorPreference(
        colors.filter(color => appointment['colorPreference'].some(cp => color['short'] === cp)).map(r => r.short)
      );
      setPriority(
        priorities.filter(priority => appointment['priority'].some(ap => priority['short'] === ap)).map(r => r.short)
      );
    })
    .catch(error => {
      console.log(error);
